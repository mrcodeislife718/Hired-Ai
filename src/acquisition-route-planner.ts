  ...
  plan(input:{organizationId?:string;opportunityId?:string;candidatePersonId?:string;fitConfidence:number;evidenceStrength:number;funnel:FunnelEvidence;applicationRequired?:boolean;hardGateMissing?:boolean}){
    ...
    if(this.graph&&input.organizationId){
      ...
      for(const item of decisionMakers.slice(0,5)){
        ...
        if(hasWarm)candidates.push({route:'warm-introduction',score:clamp(0.55+item.edge.confidence*0.25+input.evidenceStrength*0.2),reasons:['A relationship path exists to a likely hiring authority.','Warm access can bypass an application-only bottleneck.'],blockers:[],requiresApproval:true});
        if(item.edge.kind==='hiring-manager'||item.edge.kind==='functional-leader'||item.edge.kind==='founder')candidates.push({route:'direct-hiring-manager',score:clamp(0.45+item.edge.confidence*0.3+input.evidenceStrength*0.2+(noHumanSignal?0.1:0)),reasons:['A likely decision maker is identified with provenance.','Direct evidence-rich outreach is preferred when the application funnel is not producing human review.'],blockers:item.edge.confidence<0.6?['Decision-maker confidence should be verified before sending.']:[],requiresApproval:true});
