...
export class JsonFilePersistence implements PersistenceAdapter {
  ...
  constructor(path = process.env.HIRED_STATE_FILE, stateId = 'primary') {
    const id = safeKey(stateId);
    this.path = path ?? `.data/hired-state-${id}.json`;
  }
  async load() {
    try { return JSON.parse(await readFile(this.path, 'utf8')) as StoreSnapshot; }
    catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined; throw error; }
  }
  async save(snapshot: StoreSnapshot) {
...
export class PostgresPersistence implements PersistenceAdapter {
  ...
  constructor(private readonly connectionString = process.env.DATABASE_URL, private readonly stateId = 'primary') {
  ...
  async migrate() {
    ...
    await pool.query('create table if not exists hired_state (id text primary key, payload jsonb not null, updated_at timestamptz not null default now())');
  }
  async load() {
    await this.migrate();
    const pool = await this.pool();
    const result = await pool.query<{ payload: StoreSnapshot }>('select payload from hired_state where id=$1', [this.stateId]);
...
export const persistenceFromEnv = (stateId = 'primary'): PersistenceAdapter =>
  process.env.DATABASE_URL ? new PostgresPersistence(process.env.DATABASE_URL, stateId) : new JsonFilePersistence(undefined, stateId);
