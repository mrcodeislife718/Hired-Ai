...
const passwordHash = (password: string, salt: string) => scryptSync(password, Buffer.from(salt, 'hex'), 64).toString('hex');
...
export class AccountStore {
  ...
  private restore(snapshot: AccountSnapshot) {
    this.accounts.clear(); this.byEmail.clear(); this.sessions.clear();
    for (const account of snapshot.accounts ?? []) {
      this.accounts.set(account.id, account);
      this.byEmail.set(normalizeEmail(account.email), account.id);
    }
    const now = Date.now();
    for (const session of snapshot.sessions ?? []) if (Date.parse(session.expiresAt) > now) this.sessions.set(session.tokenHash, session);
  }
  ...
  async load() {
    ...
    if (this.connectionString) {
      ...
      try { this.restore(JSON.parse(await readFile(this.jsonPath, 'utf8')) as AccountSnapshot); }
  ...
  async save() {
    ...
    if (this.connectionString) {
      const pool = await this.pool();
      await pool.query('insert into hired_accounts_state(id,payload,updated_at) values($1,$2,now()) on conflict(id) do update set payload=excluded.payload, updated_at=now()', ['primary', snapshot]);
      return;
    }
  ...
  async register(email: string, password: string) {
    ...
    this.accounts.set(id, account); this.byEmail.set(normalized, id); await this.save();
    return account;
  }
  ...
  async createSession(accountId: string, days = 30) {
    ...
    const created = new Date(); const expires = new Date(created.getTime() + days * 86400_000);
    this.sessions.set(hashToken(token), { tokenHash: hashToken(token), accountId, createdAt: created.toISOString(), expiresAt: expires.toISOString() });
    await this.save();
  ...
  async updateProfile(accountId: string, input: Partial<{name:string;headline:string;skills:string[];targetLocations:string[];allowedWorkModes:WorkMode[];minBaseSalary:number;preferredTitles:string[];requiresSponsorship:boolean;}>) {
    await this.load();
    const account = this.accounts.get(accountId); if (!account) throw new Error('account not found');
    account.profile = {
      ...account.profile,
      name: input.name?.trim() || account.profile.name,
      headline: input.headline?.trim() || account.profile.headline,
      skills: Array.isArray(input.skills) ? [...new Set(input.skills.map(x=>String(x).trim()).filter(Boolean))] : account.profile.skills,
      constraints: {
        ...account.profile.constraints,
        targetLocations: Array.isArray(input.targetLocations) && input.targetLocations.length ? input.targetLocations : account.profile.constraints.targetLocations,
        allowedWorkModes: Array.isArray(input.allowedWorkModes) && input.allowedWorkModes.length ? input.allowedWorkModes : account.profile.constraints.allowedWorkModes,
        minBaseSalary: Number.isFinite(input.minBaseSalary) ? Number(input.minBaseSalary) : account.profile.constraints.minBaseSalary,
        preferredTitles: Array.isArray(input.preferredTitles) ? input.preferredTitles : account.profile.constraints.preferredTitles,
    ...
    account.updatedAt = new Date().toISOString(); await this.save(); return account;
  }

  async setSubscription(accountId: string, plan: SubscriptionPlan, status: SubscriptionStatus, customerRef?: string) {
    await this.load(); const account = this.accounts.get(accountId); if (!account) throw new Error('account not found');
    account.subscription = { plan, status, customerRef, updatedAt: new Date().toISOString() }; account.updatedAt = new Date().toISOString(); await this.save(); return account;
  }

  publicAccount(account: AccountRecord) {
    return { id: account.id, email: account.email, profile: account.profile, subscription: account.subscription, createdAt: account.createdAt };
  }
