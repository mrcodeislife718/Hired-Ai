...
for(const signal of ['SIGINT','SIGTERM'] as const)process.on(signal,()=>{server.close(()=>{void Promise.allSettled([platform.close(),employers.close(),authLimiter.close?.()??Promise.resolve(),apiLimiter.close?.()??Promise.resolve()]).finally(()=>process.exit(0));});});
