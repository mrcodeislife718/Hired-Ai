...
export interface ActionOutbox {
  ...
  delivered(id:string,workerId:string):Promise<OutboxCommand>;
  retry(id:string,workerId:string,error:string,delayMs:number,maxAttempts?:number):Promise<OutboxCommand>;
...
export class MemoryActionOutbox implements ActionOutbox {
  ...
  async enqueue(input:{aggregateType:string;aggregateId:string;action:string;idempotencyKey:string;payload:Record<string,unknown>;availableAt?:Date}){
    ...
    const at=new Date(),command:OutboxCommand={id:`outbox_${randomUUID()}`,aggregateType:input.aggregateType,aggregateId:input.aggregateId,action:input.action,idempotencyKey:input.idempotencyKey,payload:clone(input.payload),state:'pending',attempts:0,availableAt:(input.availableAt??at).toISOString(),createdAt:at.toISOString(),updatedAt:at.toISOString()};
...
export class PostgresActionOutbox implements ActionOutbox {
  ...
  async claimById(id:string,workerId:string,leaseMs=30_000,now=new Date()){
    ...
    const result=await(await this.pool()).query(`update hired_outbox set state='leased',lease_owner=$2,lease_until=$3,attempts=attempts+1,updated_at=now() where id=$1 and state in ('pending','leased') and available_at <= $4 and (lease_until is null or lease_until <= $4 or lease_owner=$2) returning *`,[id,workerId,until,now]);
  ...
  async retry(id:string,workerId:string,error:string,delayMs:number,maxAttempts=5){
    ...
    const result=await(await this.pool()).query(`update hired_outbox set state=$3,available_at=$4,lease_owner=null,lease_until=null,last_error=$5,updated_at=now() where id=$1 and lease_owner=$2 returning *`,[id,workerId,state,availableAt,error.slice(0,2000)]);if(!result.rows[0])throw new Error('outbox lease lost');return this.fromRow(result.rows[0]);
  }
