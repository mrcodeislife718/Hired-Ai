...
export function deterministicMayaReply(engine: HiredEngine, input: MayaRequest): MayaResponse {
  ...
  if (!message) return {
    ...
    type: 'welcome', coverage: surfaceCoverage(),
  ...
  if (/what can you do|capabilit|everything you can|career surfaces|well.?rounded/.test(lower)) return {
    ...
    type: 'capabilities', coverage: surfaceCoverage(),
  ...
  if (/resume|cv|positioning|outdated|career audit|profile/.test(lower)) return {
    ...
    type: 'resume-request', actions: ['Find my strongest jobs', 'Review my professional presence', 'Audit my GitHub']
  };
  ...
  if (/negotia|counter.?offer|offer package|compare.*offer|compensation package|salary offer/.test(lower)) {
    if (input.offers?.length) {
      ...
      return {
        ...
        type: 'offer-negotiation', strategy,
    ...
    return {
      ...
      type: 'offer-negotiation-request', actions: ['Compare two offers', 'Help me set a target salary', 'Prepare my negotiation script']
    };
  }
  ...
  if (/interview|mock|technical|behavioral|prepare|prep/.test(lower)) {
    ...
    return {
      ...
      type: 'interview', opportunity, readiness: pkg.readiness, interview: pkg.interview, practice: buildInterviewPractice(opportunity),
  ...
  if (/company|employer|salary|pay|culture|review|research/.test(lower)) {
    ...
    return {
      ...
      reliability: pkg.reliability, unknowns: pkg.reliability.unknowns,
  ...
  if (/apply|application|tailor|cover letter/.test(lower)) {
    ...
    if (!pkg.readiness.canOccupyRole) return {
      ...
      type: 'develop-first', opportunity, readiness: pkg.readiness, actions: ['Show me how to close the gaps', 'Find a role I can pursue now']
    };
    return {
  ...
  if (/status|today|next|pipeline|attention|follow.?up|my jobs|saved/.test(lower)) {
    ...
    return {
      ...
      type: 'status', status, actions: ['Show my best opportunity', 'Audit my resume', 'Build my network plan', 'Prepare for my next interview']
    };
  }
  ...
  if (/why|gap|weak|reject|qualified|fit|evidence|ready/.test(lower)) {
    ...
    return {
      ...
      type: 'fit', opportunity, readiness, actions: ['Prepare application', 'Prepare me for interview', 'Show a development plan']
    };
  }
  ...
  return {
    ...
    type: 'career-router', coverage: surfaceCoverage(),
...
export class MayaService {
  ...
  async respond(accountId: string, engine: HiredEngine, input: MayaRequest): Promise<MayaResponse> {
    ...
    return { ...result, message: rendered, languageModel: this.language.configured ? 'configured-with-verified-fallback' : 'deterministic-fallback', reliability: this.reliability.snapshot() };
  }
