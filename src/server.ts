...
if (process.env.HIRED_DEMO !== 'false') demoJobs.forEach(j => { try { engine.ingest(j); } catch {} });
...
const route = async (req:IncomingMessage,res:ServerResponse) => {
  try {
    ...
    if (req.method==='POST' && url.pathname==='/api/discover') {
      const result=await new DiscoveryOrchestrator(sourcesFromEnv()).run(); const ingested=[];
      for (const job of result.jobs) { try { ingested.push(engine.ingest(job)); } catch (e) { if (!(e instanceof Error && /duplicate/.test(e.message))) throw e; } }
      return json(res,200,{discovered:result.jobs.length,ingested:ingested.length,failures:result.failures});
    }
    if (req.method==='POST' && url.pathname==='/api/opportunities') return json(res,201,engine.ingest(await body(req) as RawJob));
    const pkg=url.pathname.match(/^\/api\/opportunities\/([^/]+)\/package$/); if (req.method==='GET' && pkg) return json(res,200,engine.package(pkg[1]));
    const outreach=url.pathname.match(/^\/api\/opportunities\/([^/]+)\/outreach-request$/); if (req.method==='POST' && outreach) return json(res,201,engine.requestOutreach(outreach[1]));
    const application=url.pathname.match(/^\/api\/opportunities\/([^/]+)\/application-request$/); if (req.method==='POST' && application) return json(res,201,engine.requestApplication(application[1]));
    const transition=url.pathname.match(/^\/api\/opportunities\/([^/]+)\/transition$/); if (req.method==='POST' && transition) { const b=await body(req) as {state:PipelineState}; return json(res,200,engine.governor.transition(transition[1],b.state)); }
    const feedback=url.pathname.match(/^\/api\/opportunities\/([^/]+)\/feedback$/); if (req.method==='POST' && feedback) { const b=await body(req) as Omit<FeedbackEvent,'opportunityId'>; return json(res,200,engine.recordFeedback({...b,opportunityId:feedback[1]})); }
    const approve=url.pathname.match(/^\/api\/approvals\/([^/]+)\/approve$/); if (req.method==='POST' && approve) return json(res,200,engine.governor.approve(approve[1]));
    const execute=url.pathname.match(/^\/api\/approvals\/([^/]+)\/execute$/); if (req.method==='POST' && execute) return json(res,200,{payload:engine.governor.executeApproved(execute[1])});
...
const port=Number(process.env.PORT??3000); createServer((req,res)=>{void route(req,res);}).listen(port,()=>console.log(`Hired AI listening on http://localhost:${port}`));
