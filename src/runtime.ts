...
export class HiredRuntime {
  ...
  static async create(profile: CandidateProfile, seedEvidence: Evidence[] = [], persistence = persistenceFromEnv()) {
    ...
    const engine = new HiredEngine(
      profile,
      snapshot?.evidence.length ? snapshot.evidence : seedEvidence,
      {
  ...
  startAutoCheckpoint(intervalMs = Number(process.env.HIRED_CHECKPOINT_MS ?? 5000)) {
    this.stopAutoCheckpoint();
    this.saveTimer = setInterval(() => void this.checkpoint(), intervalMs);
